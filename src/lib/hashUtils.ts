...
export const hashValues = async (values: string[]): Promise<string[]> => {
  const promises = values.map(value => hashSHA512(value));
...
export const createColumnPreview = async (
  ...
  const hashedValues = await hashValues(previewValues);
